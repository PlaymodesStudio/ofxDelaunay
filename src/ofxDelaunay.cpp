...
void ofxDelaunay::setPointAtIndex(const ofDefaultVec3& p, int index) {
...
// ------------------------------------------------------------------------------------------------------
ofDefaultVec3 ofxDelaunay::getPointNear(const ofDefaultVec3& pos, float minimumDist, int& index) {
	XYZI ret;
...
std::vector<ofDefaultVec3> ofxDelaunay::getPointsForITriangle(const ITRIANGLE& triangle) {
...
// ------------------------------------------------------------------------------------------------------
ITRIANGLE ofxDelaunay::getTriangleForPos(const ofDefaultVec3& pos) {
...
int ofxDelaunay::triangulate() {
	...
	for (int i = 0; i < nv; i++) {
		XYZ v;
		v.x = verticesTemp[i].x;
		v.y = verticesTemp[i].y;
		v.z = verticesTemp[i].z;
