...
public:
	...
	int addPoints(const std::vector<ofDefaultVec3>& points);

	/// \brief setPointAtIndex invalidates mesh
	void setPointAtIndex(const ofDefaultVec3& p, int index);
	...
	ofDefaultVec3 getPointNear(const ofDefaultVec3& pos, float minDist, int& index);

	/// \return returns ITRIANGLE(0,0,0) if none found!
	ITRIANGLE getTriangleForPos(const ofDefaultVec3& pos);
